...
function UploadTms() {
  ...
  const [show, setShow] = useState({ file1: false, file2: false });
  ...
  const onSubmit = async (data: any) => {
    ...
    setIsLoading(true);
    const { meroshare, tms } = data;
    console.log(meroshare, tms);

    const formData = new FormData();

    formData.append("file", tms);
  ...
  return (
    <>
      ...
      <Dialog>
        <DialogTrigger className="text-[#0b0b0b] bg-white hover:bg-black hover:text-white  w-full py-2 transition-colors hover:text-white-500 cursor-pointer border-b hover:border-b-black-500">
          upload Tms file
        </DialogTrigger>
        <DialogContent className="bg-black border border-[#807f7f] px-[15px] py-4 m-0 text-[#fff]">
          <DialogHeader>
            ...
            <DialogDescription>
              Import tms transaction history/ account statements sent by your
              broker.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            ...
            >
              <FormField
                ...
                render={({ field: { value, onChange, ...fieldProps } }) => (
                  <FormItem className="relative">
                    ...
                    <div className="bg-transparent w-full text-white h-[88px] -z-10">
                      {value ? (
                        <label
                          htmlFor="tms"
                          className="w-full h-full px-2 grow-0 flex items-center justify-between gap-2 bg-[#9b9b9b1a] border-[#9b9b9b4d] border text-[#c5c5c5] cursor-pointer"
                        >
                      ...
                      <div className="absolute flex items-center justify-between px-[18px] rounded-[5px] z-50 top-[23.9px] left-0 h-[90px] text-[#717171] w-full bg-[#202020] border-2 border-[#9b9b9b4d]">
                        <div className="flex gap-2">
                          ...
                          <div className="flex flex-col gap-1">
                            <h1 className="overflow-ellipsis ... overflow-clip w-[200px]">
                              {value.name}
                            </h1>
                            <p>{value.size}</p>
                          </div>
                    ...
                    <FormControl>
                      <Input
                        ...
                        accept=".xlsx"
                        id="tms"
                        className="bg-transparent text-white h-[88px] -z-10 hidden "
                        onChange={(event) => {
                          onChange(event.target.files && event.target.files[0]);
                          setShow((prev) => {
                            return { ...prev, file2: true };
                          });
                        }}
                      />
                    </FormControl>
              ...
              <div className="flex items-center gap-1">
                ...
                <a
                  ...
                  href="https://www.canva.com/design/DAGPqsoqndg/sXvb0YR9zdVjFPAWK6a3Fw/view"
                  target="_blank"
